 ...
 *
 *AGN Modified by Tony Nicholson to detect end-of-file sector type
 *
 */
#include <stdio.h>
#include <window.h>
...
unsigned char *search_form[] = {		// Text entry form for search string
	76<<8|3,
	"\x01\x00\x40String?",
	0 };

// Command line help
unsigned char help[] = { "Copyright 2007 Dave Dunfield - All rights reserved.\n\
...
^Home/^End	= First/Last sector on disk\n\
 PgUp/PgDn	= Page data display\n\
   ENTER	= Edit sector\n\
	...
	F4		= Search again\n\
	F1		= Display all strings from current track onward\n\
  ESC/F10	= Exit" };
...
int Gete()
{
	...
	if(!read(&c, 1, fh))
		error("Corrupt image file in Gete");
	if(!++L)
		++H;
	return c;
}
...
void Getb(unsigned char *dest, unsigned size)
{
	...
	if(size) {
		if(read(dest, size, fh) != size)
			error("Corrupt image file in Getb");
		i = L;
		L += size;
		if(L < i)
			++H; }
...
int read_track(void)
{
	...
	if((i = Getc()) == EOF)			// Mode byte if another track
        ...
        if( i == 26 ) return -1;    // AGN - if mode byte 26 then End-of-File
...
void find(char again)
{
	...
	status("Search: %s", Fstring);
	if(again)		// Repeated search
		goto findagain;

	// Initial search, note the length & clear the circular buffer
	Slen = strlen(Fstring);
		...
		for(Fs = 0; Fs < Nsect; ++Fs) {			// Each sector
			...
			for(Fp=0; Fp < Secsize; ++Fp) {		// Each byte
				...
				if(Charset)						// Charcter set translation
					c = ebcdic[c];
				Sbuffer[Spos] = Csearch ? toupper(c) : c;
				if(++Spos >= Slen)
					Spos = 0;
...
void edit_charset(unsigned char *file)
{
	...
	wcursor_off();
	status("ImageDisk Viewer "#VERSION" / "#__DATE__" - Copyright 2007 Dave Dunfield");
...
main(int argc, char *argv[])
{
		...
		case 'C=' :									// Custom character set
			...
			while(*p = toupper(c = *p)) {
				...
				if(c == '\\') {
					...
					continue; } }
			continue;
		case 'E=' : edit_charset(p);	return;		// Edit character set
		case 'M=' : Mstring = atoi(p);	continue;	// Minimum string length
		case 'S=' :									// Scan strings to file
			filename(p, ".TXT");
			fps = fopen(Omap, "wvq");
			continue;
		case 'W=' : Width = atoi(p);	continue;	// Output width
		case 'X=' : Xor = atox(p);		continue;	// Data XOR value
		} if(File)
			abort(help);
		File = p-2; }
	...
	while((c = Gete()) != 0x1A) {
		if(c == EOF)
			abort("Corrupt image file in main");
		putc(c, stdout); }
...
redraw:			// Redraw screen
	...
	for(;;) {	// Command loop - interpret keypresses
		c = wgetc();
		if(Sflag) {		// Status changed - reshow default
			status("ImageDisk Viewer: F1=Help  F2=Charset  F3=Search  F4=Again  F5=Strings  F10=Exit");
			Sflag = 0; }
		switch(c) {
		...
		case _K3 :											// First search
			if(wform(2, 10, WSAVE|WCOPEN|WBOX1|0x70, search_form, Fstring))
				continue;
			if(Csearch)
				strupr(Fstring);
			c = 0;
		case _K4 :											// Repeat search
			if(*Fstring) {
				...
				goto new_track; }
			continue;
		case _K5 : wclwin(); find_strings(); goto new_track;	// Strings
		case _K10:
		case 0x1B:											// Exit
			close(fh);
			wclose();
